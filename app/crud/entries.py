...
async def c_user_entry(entry: EntryCreate, create_by: PydanticObjectId | None) -> Entry:
    db_entry = Entry(**entry.dict())
    db_entry.create_by = create_by
    db_entry.create_time = datetime.utcnow()
    await Entry.insert_one(db_entry)
    return db_entry
...
async def r_user_entries(user_id: PydanticObjectId | None) -> list[Entry]:
    db_entrys = await Entry.find({"create_by": user_id}).to_list()
    return db_entrys
...
async def r_user_entry(
    ...
    db_entry = await Entry.find_one({"create_by": user_id, "_id": entry_id})
    if db_entry is None:
        raise HTTPException(
...
async def d_user_entry(
    ...
    await Entry.find_one({"_id": entry_id, "create_by": user_id}).delete()
